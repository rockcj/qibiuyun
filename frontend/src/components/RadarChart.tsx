...
function polygonPoints(total: number, value: number, center: number, radius: number): string {
    ...
    <div className={`flex justify-center ${className}`}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="overflow-visible">
        ...
        {gridLevels.map((level) => (
          <polygon key={level} points={polygonPoints(total, level, center, radius)}
            fill="none" stroke="currentColor"
            className="text-zinc-200 dark:text-zinc-700" strokeWidth="1" />
        ))}
        {/* 轴线 */}
        {Array.from({ length: total }, (_, i) => {
          const v = getVertex(i, total, 1, center, radius);
          return <line key={i} x1={center} y1={center} x2={v.x} y2={v.y}
            stroke="currentColor" className="text-zinc-200 dark:text-zinc-700" strokeWidth="1" />;
        })}
        {/* 分数多边形 */}
        <polygon
          ...
          fill="rgba(99, 102, 241, 0.15)" stroke={INDIGO} strokeWidth="2" />
        {/* 分数圆点 */}
        {dimensions.map((d, i) => {
          const v = getVertex(i, total, d.score / maxScore, center, radius);
          return <circle key={i} cx={v.x} cy={v.y} r="4" fill={INDIGO} />;
        })}
        {/* 标签 */}
        {dimensions.map((d, i) => {
          const v = getVertex(i, total, 1.15, center, radius);
          return (
            <text key={i} x={v.x} y={v.y} textAnchor="middle" dominantBaseline="middle"
