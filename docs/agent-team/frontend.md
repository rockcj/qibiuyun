...
本文档面向前端 Agent，负责把 OfferGPT 的产品流程转化为可演示、可交互、可回放的 Web Demo。前端优先使用 Next.js 实现，P0 阶段必须先完整落地面试场景闭环，餐厅点餐和商务会议仅作为后续增量模块加入，不能影响面试场景的正常使用。

## 开发优先级

| 优先级 | 场景模块 | 前端目标 | 进入下一阶段条件 |
|---|---|---|---|
| P0 | 面试场景 | 完成简历上传、JD 输入、Persona 选择、实时面试、轻纠正、报告和 VAR | 面试闭环可独立演示并通过测试 |
| P1 | 餐厅点餐 | 在不破坏面试闭环的前提下新增点餐主题、服务员角色和点餐报告 | P0 面试回归测试通过 |
| P2 | 商务会议 | 在 P0/P1 稳定后新增会议主题、会议角色和会议报告 | P0/P1 回归测试通过 |
...
| 模块 | 前端职责 | 不负责内容 |
|---|---|---|
| 场景选择 | P0 展示面试入口，P1/P2 可展示点餐和会议占位入口 | 后端场景规则计算 |
...
| 页面 | 路由建议 | 核心功能 |
|---|---|---|
| 首页 | `/` | P0 优先展示面试入口，点餐和会议可显示“即将开放” |
| 场景配置页 | `/scenes/[scene]` | P0 只要求面试 Persona、难度和纠错配置 |
...
// 场景类型必须与后端接口保持一致，P0 仅实现 interview
...
- P0 阶段面试场景可以从首页进入并完成配置。
- 面试场景可以上传简历、输入 JD、选择 Persona 并创建会话。
- 实时面试页可以展示用户字幕、AI 面试官文本回复和轻纠正提示。
- 面试报告页可以展示 Offer Score、分项评分、建议和 VAR 时间轴。
- 餐厅点餐和商务会议未开发时，只允许作为占位入口存在，不得影响面试闭环。
- 无麦克风或后端异常时，面试预置 Demo 仍能完成演示闭环。
