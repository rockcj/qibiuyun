...
本文档面向前端 Agent，负责把 OfferGPT 的产品流程转化为可演示、可交互、可回放的 Web Demo。前端优先使用 Next.js 实现，重点保证三类场景入口、实时语音对话、轻纠正提示、VAR 时间轴和场景报告可以稳定展示。
...
| 模块 | 前端职责 | 不负责内容 |
|---|---|---|
| 场景选择 | 展示面试、餐厅点餐、商务会议入口 | 后端场景规则计算 |
...
| 页面 | 路由建议 | 核心功能 |
|---|---|---|
| 首页 | `/` | 项目介绍、三场景卡片、开始训练入口 |
| 场景配置页 | `/scenes/[scene]` | 子主题、角色、难度、Persona 配置 |
...
| Demo 兜底页 | `/demo` | 预置演示数据、无麦克风时可直接演示 |
...
用户进入首页
  ↓
选择 interview / restaurant / meeting
  ↓
填写场景配置
  ↓
面试场景补充简历和 JD
...
- 三个场景都能从首页进入配置页。
- 面试场景可以上传简历并输入 JD。
- 实时对话页可以展示用户字幕、AI 文本回复和轻纠正提示。
- 报告页可以展示场景总分、分项评分、建议和 VAR 时间轴。
