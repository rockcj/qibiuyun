...
async def _save_grammar_timeline_event(
    session_id: str,
    turn_id: str,
    grammar_result,
    transcript: str,
) -> None:
    """写入语法错误时间轴事件到数据库（后台任务，不阻塞主链路）。"""
    from database import async_session_factory
    ...
    try:
        async with async_session_factory() as db:
            ...
            db.add(TimelineEvent(
                ...
                description=(
                    f"Original: '{grammar_result.original}' → "
                    f"Corrected: '{grammar_result.corrected}'"
                ),
                start_ms=estimated_start_ms,
                end_ms=estimated_end_ms,
                transcript_snippet=transcript[:200] if transcript else "",
                evidence={
                    "original": grammar_result.original,
                    "corrected": grammar_result.corrected,
                    "tip": grammar_result.spoken_tip,
                },
