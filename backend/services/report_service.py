    ...
    def _compute_scores(
        ...
        for dim in rubric:
            if dim in base_dimensions:
                ...
                dimension_scores[dim] = _clamp(
                    0.5 * base_dimensions["english"] + 0.5 * base_dimensions["grammar"] - 5
                )
            elif dim == "logic":
                dimension_scores[dim] = _clamp(
                    0.4 * base_dimensions["english"] + 0.6 * base_dimensions["grammar"]
                )
            elif dim == "technical":
                ...
                dimension_scores[dim] = _clamp(
                    0.5 * base_dimensions.get("pronunciation", 70)
                    + 0.5 * base_dimensions.get("fluency", 70)
                )
            else:
